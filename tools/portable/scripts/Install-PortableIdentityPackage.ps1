...
if ($AllowUnsigned) {
    ...
    if ([string]::IsNullOrWhiteSpace($publisher) -or $publisher -notmatch [regex]::Escape($unsignedPackagePublisherOid)) {
        throw "The sparse identity package publisher must include $unsignedPackagePublisherOid when using -AllowUnsigned. Rebuild the package with New-PortableIdentityPackage.ps1."
    }
