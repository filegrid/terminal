...
namespace
{
    constexpr std::wstring_view UiLanguageOverrideEnvironmentVariable{ L"WT_UI_LANGUAGE_OVERRIDE" };

    std::wstring _resolvePreferredLanguageOverride(std::wstring_view configuredLanguage)
    {
        if (const auto overrideLanguage = wil::TryGetEnvironmentVariableW<std::wstring>(UiLanguageOverrideEnvironmentVariable.data());
            !overrideLanguage.empty())
        {
            return overrideLanguage;
        }

        return std::wstring{ configuredLanguage };
    }
}
...
namespace winrt::TerminalApp::implementation
{
    ...
    try
    {
        const auto language = _resolvePreferredLanguageOverride(_settings.GlobalSettings().Language().c_str());
