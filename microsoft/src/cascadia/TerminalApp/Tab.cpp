...
namespace winrt::TerminalApp::implementation
{
    ...
    void Tab::_CreateContextMenu()
    {
        ...
        {
            Controls::FontIcon inputPanelSymbol;
            inputPanelSymbol.FontFamily(Media::FontFamily{ L"Segoe Fluent Icons, Segoe MDL2 Assets" });
            inputPanelSymbol.Glyph(L"\xE70F");
            ...
            _workspaceInputPanelMenuItem.Click([weakThis](auto&& sender, auto&&) {
                if (auto tab{ weakThis.get() })
                {
                    if (const auto toggle = sender.try_as<Controls::ToggleMenuFlyoutItem>())
                    {
                        tab->ShowWorkspaceInputPanel(toggle.IsChecked());
                    }
                }
            });
        }
        ...
        contextMenuFlyout.Items().Append(_workspaceInputPanelMenuItem);
        ...
        contextMenuFlyout.Opening([weakThis](auto&& sender, auto&&) {
            if (auto tab{ weakThis.get() })
            {
                if (const auto flyout = sender.try_as<Controls::MenuFlyout>())
                {
