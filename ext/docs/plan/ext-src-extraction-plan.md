...
| portable 构建 / 打包 | `New-UnpackagedTerminalDistribution.ps1`、`CascadiaPackage.wapproj`、顶层 `CMakeLists.txt` |
| 低层补丁 | `til\mutex.h` |
...
### 原则 4：构建脚本也按“入口/实现”拆分

- 原始脚本/工程保留兼容入口职责
- 真正的 portable 定制实现尽量放到 `ext\src\portable`
- 原始脚本只做参数透传、挂接、兼容，不承载新业务
...
**目标：** portable 的“实现层”放 `ext\src\portable`，原始树只保留兼容挂接
...
- `New-UnpackagedTerminalDistribution.ps1` 作为旧路径兼容入口
- `CascadiaPackage.wapproj` 中不可避免的项目/打包接线
...
| `microsoft\src\cascadia\CascadiaPackage\CascadiaPackage.wapproj` | 必留原始树 | WAP 项目文件必须在原项目树内 | 仅保留打包接线与兼容修正 |
| `microsoft\build\scripts\New-UnpackagedTerminalDistribution.ps1` | 必留原始树 | 旧构建入口要兼容现有调用面 | 只保留兼容壳，新增实现不继续堆这里 |
