    void TerminalPage::_DispatchWorkspaceChatInput(const TermControl& control, const std::wstring_view text)
    {
        ...
        const auto useSpecialSubmitPath = bracketedPasteEnabled || _ShouldUseTwoPhaseWorkspaceChatSubmit(control);
        ...
        logPayload["useSpecialSubmitPath"] = useSpecialSubmitPath;
        ...
        if (_workspaceExtension->WorkspaceChatSubmitTransport() == WorkspaceChatSubmitTransport::WindowKeyboardInput)
        {
            if (useSpecialSubmitPath)
            {
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "submitPayload", L"<window-keyboard-enter>");
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "combinedPayload", terminal::workspacechat::NormalizeWorkspaceChatSubmitText(text));
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "pastePayload", terminal::workspacechat::NormalizeWorkspaceChatSubmitText(text));
                logPayload["dispatchMode"] = "window-keyboard-br-normalized-enter";
                logPayload["twoPhaseSend"] = true;
                _logWorkspaceChatDiagnostic(L"chat_dispatch_send_input", logPayload);
                _DispatchWorkspaceChatWindowKeyboardInput(control, terminal::workspacechat::NormalizeWorkspaceChatSubmitText(text), true);
            }
            else
            {
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "pastePayload", L"");
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "submitPayload", L"<window-keyboard-enter>");
                terminal::workspacechat::AddDiagnosticTextFields(logPayload, "combinedPayload", text);
                logPayload["dispatchMode"] = "window-keyboard-enter";
                logPayload["twoPhaseSend"] = true;
                _logWorkspaceChatDiagnostic(L"chat_dispatch_send_input", logPayload);
                _DispatchWorkspaceChatWindowKeyboardInput(control, std::wstring{ text }, true);
            }
        }
        else if (useSpecialSubmitPath)
        {
            std::wstring pastePayload{ text };
            if (bracketedPasteEnabled)
            {
                pastePayload.insert(0, bracketedPasteBodyPrefix);
            }
            pastePayload.append(pasteBodySuffix);
            std::wstring combinedPayload{ pastePayload };
            combinedPayload.append(submitPayload);
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "pastePayload", pastePayload);
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "submitPayload", submitPayload);
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "combinedPayload", combinedPayload);
            logPayload["dispatchMode"] = bracketedPasteEnabled ? "paste-leading-lf-trailing-lf-sendinput-cr" : "paste-lf-sendinput-cr";
            logPayload["twoPhaseSend"] = true;
            _logWorkspaceChatDiagnostic(L"chat_dispatch_send_input", logPayload);
            control.PasteText(winrt::hstring{ pastePayload });
            control.SendInput(winrt::hstring{ submitPayload });
        }
        else
        {
            std::wstring sendInputPayload{ text };
            sendInputPayload.append(submitPayload);
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "pastePayload", L"");
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "submitPayload", sendInputPayload);
            terminal::workspacechat::AddDiagnosticTextFields(logPayload, "combinedPayload", sendInputPayload);
            logPayload["dispatchMode"] = "sendinput-inline-cr";
            logPayload["twoPhaseSend"] = false;
            _logWorkspaceChatDiagnostic(L"chat_dispatch_send_input", logPayload);
            control.SendInput(winrt::hstring{ sendInputPayload });
        }
    }
