...
internal sealed class Program
{
    ...
    private sealed class PortablePackageTool
    {
        ...
        public void Run()
        {
            ...
            var package = FindTerminalPackage();
            var xamlPackage = FindXamlPackage(package);
        ...
        private FileInfo FindTerminalPackage()
        {
            var candidates = new DirectoryInfo(_options.PackageRoot)
                .EnumerateFiles("CascadiaPackage_*.msix", SearchOption.AllDirectories)
                .Where(file => !file.FullName.Contains($"{Path.DirectorySeparatorChar}Dependencies{Path.DirectorySeparatorChar}", StringComparison.OrdinalIgnoreCase))
                .Where(file => MatchesConfiguration(file.FullName))
                .OrderByDescending(file => file.LastWriteTimeUtc)
                .ToList();

            if (candidates.Count == 0)
            {
                throw new InvalidOperationException($"Could not find a CascadiaPackage msix under {_options.PackageRoot}.");
            }

            return candidates[0];
        }

        private FileInfo FindXamlPackage(FileInfo terminalPackage)
        {
            var dependencyDirectory = Path.Combine(terminalPackage.DirectoryName!, "Dependencies", _options.Platform);
            if (!Directory.Exists(dependencyDirectory))
            {
                throw new InvalidOperationException($"Could not find the dependency directory {dependencyDirectory}.");
            }

            var candidate = new DirectoryInfo(dependencyDirectory)
                .EnumerateFiles("Microsoft.UI.Xaml*.appx", SearchOption.TopDirectoryOnly)
                .OrderByDescending(file => file.LastWriteTimeUtc)
                .FirstOrDefault();

            if (candidate == null)
            {
                throw new InvalidOperationException($"Could not find the Microsoft.UI.Xaml dependency package for {_options.Platform}.");
            }

            return candidate;
        }

        private bool MatchesConfiguration(string path)
        {
            if (string.Equals(_options.Configuration, "Release", StringComparison.OrdinalIgnoreCase))
            {
                return !path.Contains("_Debug_", StringComparison.OrdinalIgnoreCase);
            }

            return path.Contains($"_{_options.Configuration}_", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void BuildSingleFilePortable(string distributionName, string version, string architecture, string outputZip)
        {
            ...
            try
            {
                ...
                RunProcess(_options.CscPath,
                    $"/nologo /target:winexe /optimize+ /platform:{cscPlatform} /win32icon:\"{launcherIcon}\" /r:System.IO.Compression.dll /r:System.IO.Compression.FileSystem.dll /r:System.Windows.Forms.dll /out:\"{launcherPath}\" \"{assemblyInfo}\" \"{launcherSource}\"");

                var outputExe = Path.Combine(_options.Destination, distributionName + ".exe");
                ...
                PublishSystemAlias(new FileInfo(outputExe), version, architecture);
            }
            finally
            {
        ...
        private void PublishSystemAlias(FileInfo portableArtifact, string version, string architecture)
        {
            ...
            {
                $"WindowsTerminalPortable_en-US{configurationSuffix}_{version}_{architecture}{portableArtifact.Extension}",
                $"WindowsTerminalPortable_zh-CN{configurationSuffix}_{version}_{architecture}{portableArtifact.Extension}",
                $"WindowsTerminalPortableGeekEdition_System{configurationSuffix}_{version}_{architecture}{portableArtifact.Extension}",
                $"WindowsTerminalPortableGeekEdition_English{configurationSuffix}_{version}_{architecture}{portableArtifact.Extension}"
            };

            foreach (var alias in aliasesToDelete)
            {
                var aliasPath = Path.Combine(portableArtifact.DirectoryName!, alias);
            ...
            var systemAlias = $"WindowsTerminalPortableGeekEdition_System{configurationSuffix}_{version}_{architecture}{portableArtifact.Extension}";
            var systemAliasPath = Path.Combine(portableArtifact.DirectoryName!, systemAlias);
            if (File.Exists(systemAliasPath))
            {
                File.Delete(systemAliasPath);
            }

            File.Copy(portableArtifact.FullName, systemAliasPath);
        }
...
""";
    ...
    private sealed class Options
    {
        ...
        public required string CscPath { get; init; }
        ...
        public static Options Parse(string[] args)
        {
            ...
            return new Options
            {
                PackageRoot = GetRequired(values, "--package-root"),
                ...
                CscPath = GetRequired(values, "--csc-path"),
