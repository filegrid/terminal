    ...
    bool _hasSshTtyOption(std::wstring_view commandline)
    {
        ...
        const auto commandlineBuffer = std::wstring{ commandline };
        auto argc = 0;
        wil::unique_any<LPWSTR*, decltype(&::LocalFree), ::LocalFree> argv{ CommandLineToArgvW(commandlineBuffer.c_str(), &argc) };
        if (!argv)
        {
            return _containsInsensitive(commandline, L" -t") || _containsInsensitive(commandline, L" -tt");
        }

        for (auto index = 1; index < argc; ++index)
        {
            const auto arg = std::wstring_view{ argv.get()[index] };
            if (arg == L"-t" || arg == L"-tt")
            {
                return true;
            }
