    ...
    void TerminalPage::_LoadWorkspaceEditorState(const bool preserveSelection)
    {
        ...
        _workspaceExtension->WorkspaceEditorManager() = Microsoft::Terminal::Settings::Model::implementation::WorkspaceManager::Load();
        ...
        if (!selectedWorkspaceId.empty())
        {
            ...
            {
                _workspaceExtension->WorkspaceEditorSelectedIndex() = gsl::narrow_cast<size_t>(std::distance(workspaces.begin(), it));
                return;
            }
        }
        ...
        _workspaceExtension->WorkspaceEditorSelectedIndex() = std::min(_workspaceExtension->WorkspaceEditorSelectedIndex(), workspaces.size() - 1);
    }
